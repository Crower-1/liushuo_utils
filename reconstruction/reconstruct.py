...
class WBPReconstructor(TomoReconstructor):
    ...
    def _r_weight(self, sinogram):
        """
        Ram-Lak on sinogram (with Hamming)
        """
...
def _backproject_numba(sinogram, cos_sin_values, proj_x, vol_z, scale=1):
    """
    sinogram : (n_tilts, proj_x)
    cos_sin_values : 预计算的余弦和正弦值 (n_tilts, 2)
    proj_x : 投影宽度

    volume_slice : 重构的 2D 切片 (proj_x, vol_z)
    count_slice : 每个体素贡献的累计计数
    """
...
def main():
    ...
    parser.add_argument('-t', '--tilt_file', required=True, help='tilt angle file, each line is a angle in degree')
