...
3. label中的mask原本全部为1，需要在3D中对mask做连通性分析，将每个独立的连通区域分别标记为1,2,...n。
4. 对最终切片时，不再输出全部z层，只选择mask在z轴方向上有标记的最小和最大层之间的所有z层进行切片(包含最小和最大)，图像切片存放到 ./train/JPEGImage/{base_name}/，标签切片存放到 ./train/Annotations/{base_name}/。
...
def process_image_label_pair(image_path, label_path, base_name):
    ...
    # 对mask进行3D连通性标记
    # mask中的前景假设为>0的像素，这里原始mask全为1的区域为前景
    print("对mask进行3D连通性分析...")
    structure = np.ones((3,3,3), dtype=bool)
    labeled_mask, num_features = cc3d_label(mask_resized, structure=structure)
    print(f"发现 {num_features} 个连通区域。")
    ...
    with mrcfile.new(label_mrc_path, overwrite=True) as mrc:
        mrc.set_data(labeled_mask.astype(np.int16))

    # 找到label中有标记的z范围
    nonzero_z = np.where(labeled_mask > 0)[0]
    if len(nonzero_z) == 0:
    ...
    for z_idx in range(z_min, z_max + 1):
        ...
        # 处理mask切片
        mask_slice = labeled_mask[z_idx, :, :].astype(np.uint16)
