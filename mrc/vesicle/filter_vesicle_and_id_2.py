...
def extract_tether_contact_vesicle_ids(xml_path):
    # 解析XML文件
    ...
    for vesicle in root.findall('Vesicle'):
        ...
        if type_tag is not None and type_tag.get('t') == 'tether':
            tether_vesicle_ids.append(int(vesicle_id))  # 添加vesicleId到列表
        elif type_tag is not None and type_tag.get('t') == 'contact':
            contact_vesicle_ids.append(int(vesicle_id))
    
    # 返回'others'类型的vesicleId列表
    return tether_vesicle_ids, contact_vesicle_ids

def generate_vesicle_paths(base_path, base_name):
    mrc_path = f'{base_path}/{base_name}/ves_seg/{base_name}_label_vesicle.mrc'
    json_path = f'{base_path}/{base_name}/ves_seg/{base_name}_vesicle.json'
    xml_path = f'{base_path}/{base_name}/ves_seg/vesicle_analysis/{base_name}_vesicle_class.xml'
    # xml_path = f'/media/liushuo/data1/data/fig_demo/{base_name}/ves_seg/vesicle_analysis/{base_name}_ori.xml'
    small_vesicle_output = f'{base_path}/{base_name}/ves_seg/filter_vesicle/small_vesicles.mrc'
    other_vesicle_output = f'{base_path}/{base_name}/ves_seg/filter_vesicle/other_vesicles.mrc'
    contact_id_output = f'{base_path}/{base_name}/ves_seg/filter_vesicle/contact_vesicles.mrc'
    tether_id_output = f'{base_path}/{base_name}/ves_seg/filter_vesicle/tether_vesicles.mrc'
...
if __name__ == "__main__":
    ...
    base_path = '/media/liushuo/data1/data/fig_demo_2/'
    base_name = "pp199"
    mrc_path, json_path, xml_path, small_vesicle_output, other_vesicle_output, contact_id_output, tether_id_output = generate_vesicle_paths(base_path, base_name)


    threshold = 0.1
    # threshold = 34 / 1.714 / 2
    
    tether_id_list, contact_id_list = extract_tether_contact_vesicle_ids(xml_path)
