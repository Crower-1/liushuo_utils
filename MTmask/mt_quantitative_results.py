...
def load_mt_json(json_path):
    with open(json_path, "r", encoding="utf-8") as f:
        data = json.load(f)
    if isinstance(data, dict):
        data = data.get("data", [])
    if not isinstance(data, list):
        raise ValueError("JSON content must be a list of mt objects.")
    return data
...
def make_vector_field_plot(df, output_path, scale):
    ...
    ax.set_ylabel("Y (nm)")
    ax.set_zlabel("Z (nm)")
    ax.set_title("mt orientation vectors")
    fig.tight_layout()
    fig.savefig(output_path, dpi=150)
    plt.close(fig)
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute mt quantitative metrics from JSON points."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=24.0,
        help="mt filament diameter in nm (default: 24).",
    )
    parser.add_argument(
        "--vector-scale",
        type=float,
    ...
    data = load_mt_json(args.json_path)
    df = compute_metrics(data, args.pixel_size, args.diameter_nm)
    if df.empty:
        print("No valid mt entries found. No output generated.")
        return
